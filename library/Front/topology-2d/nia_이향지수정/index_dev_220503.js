    ...
    ; (function (global) {
        ...
        let Map2d = global.Map2d = function () {
            ...
            let defaultOptions = {
                ...
                node: { displayField: 'device_name', fix_size: false, displayImage: 'image', show_r: false, r: 30, width: 40, height: 35, display_badge: true, badge_size: 30, drag_weight: 2, stroke_color: "#604c45", color: "#634d45", opacity: 0.7, text_size: 20 },
                ...
                link_traffic: { show: true, show_arrow: false, 'r': 3, color: "#555", arrow_color: '#ff0606', opacity: 1.0, dur: 5 },
                filePath: 'data/',
            };
            let options =
                Object.assign(objectCopyPrimitive(defaultOptions), {
                    fileName: 'data_nia_1.json',
                    link_type: 'line',
                    save,
                    load,
                    exportToFile,
            ...
            function onChangeNodeBadgeSize() {
                ...
                svg.selectAll('image.node_badge')
                    .attr('transform', function (d) {
                ...
                svg.selectAll('g.node_badge')
                    .classed(className.invisible, !options.node.display_badge);
            }
            ...
            function loadConfig(data) {
                ...
                const ZOOM_INIT = data.config.zoom.init != undefined;
                ...
                if (ZOOM_INIT) {
                    setTimeout(() => {
                        ctrl.generateZoomIn(data.config.zoom.init);
                    }, 1000)
                }
            }
        ...
        Map2d.prototype = {
            ...
            drawLinkHandler: function (element, type, d, index, all) {
                ...
                let drawLine = function (d, directional) {
                    ...
                    } else { return ("M" + (x1 + point.x) + "," + (y1 + point.y) + "L" + " " + (x2 + point.x) + "," + (y2 + point.y)); }
        ...
        window.onload = () => { return;
            ...
            setTimeout(function () {
                ...
                gui.add(options, 'fileName', ['data.json', 'data_line.json', 'data_mba.json', 'data_mba_1.json', 'data_mba_2.json', 'data_stable.json', 'data_nia_1.json', 'data_nia_1_1.json', 'data_nia_1_2.json', 'data_nia_2.json', 'data_nia_2_1.json', 'data_nia_2_2.json', 'data_nia_2_3.json', 'data_nia_2_4.json', 'data_nia_2_5.json', 'data_nia_2_6.json', 'data_nia_3.json']).name('파일').onChange(e => { options.save_map = false; options.load(e) }).listen();
