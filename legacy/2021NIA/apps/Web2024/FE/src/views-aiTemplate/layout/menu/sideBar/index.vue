<template>
  <el-aside id="side-menu" class="p-0" :style="{ width: getWidth, 'z-index': isActive? 2 : 1}">
    <el-scrollbar wrap-class="scrollbar-wrapper" class="w-full h-full">
      <el-menu class="h-full" menu-trigger="click" :collapse-transition="false" @open="handleOpen" @close="handleClose">
        <!--
          ...
          위의 error로 인해 사용 x
         -->
        <SideBarItem v-for="route in routes" v-if="!route.hidden" :key="route.path" :base-path="route.path" :item="route" />
      </el-menu>
    </el-scrollbar>
  </el-aside>
