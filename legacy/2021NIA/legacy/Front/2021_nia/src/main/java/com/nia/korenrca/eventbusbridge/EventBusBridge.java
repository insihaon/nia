package com.nia.korenrca.eventbusbridge;
...
public class EventBusBridge {
    ...
    private EventBusBridge(int port, String host, URI endPoint, io.vertx.core.Handler<EventBusBridge> onOpenHandler, HttpClientOptions options, Optional<Vertx> aVertx) {
        ...
        vertx.createHttpClient(options).websocket(port, host, endPoint.toString() + "/websocket", ws -> {
            LOGGER.debug(String.format(endPoint.toString() + "/websocket => WEBSOCKET CONNECT SUCCESS !!"));
            webSocket = ws;
    ...
    private static String getUrl() {
        return "http://127.0.0.1:" + ConfigProperties.get().getSocketServerPort() + Constants.EVENT.EventBusUri;
    }

    private static volatile EventBusBridge server;
    public static EventBusBridge getServer() {
        EventBusBridge ref = EventBusBridge.server;
        try {
            if(ref == null) {
                synchronized (EventBusBridge.class) {
                    if(EventBusBridge.server == null) {
                        LOGGER.debug(String.format(" CREATE NEW EventBusBridge Connection"));
                        URI uri = URI.create(getUrl());
                        ref = EventBusBridge.server = EventBusBridge.connect(uri, eb -> { });
                        Thread.sleep(500);
                    }
                }
            }
        } catch (Exception e) {
            LOGGER.error(String.format(" EventBusBridge getServer() EXCEPTION " + e));
        }
        return ref;
    }

    public static void connectionFailed() {
        LOGGER.debug(String.format(" EventBusBridge Connection Failed !!"));
        EventBusBridge.server = null;
    }

    /* getServer() 사용법
        try{
            EventBusBridge.getServer().publish("in",new JsonObject().put("msg","111"));
