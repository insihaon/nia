...
public class RoadmSingleRule {
	private final Logger LOGGER = Logger.getLogger(RoadmSingleRule.class);
	...
	public RcaResult roadmAlarmRule(List<BasicAlarmVo> inputAlarmList){
		LOGGER.info(">>>>>>>[RoadmSingleRule] roadmSingleRule <<<<<<<<<<<<");
		try {
			RcaResult roadmRcaResult = new RcaResult();
			Flag flag = new Flag();
			kieContainer = KnowledgeSessionHelper.createRuleBase();

			// session 및 전역변수 설정부
			sessionStatefull = KnowledgeSessionHelper.getStatefulKnowledgeSession(kieContainer, "ksession-roadm");

			sessionStatefull.setGlobal("rcaResult", roadmRcaResult);
			...
			LOGGER.info(">>>>>>>[roadmSingleRule] roadmAlarmRule inputroadmAlarmList : " + inputAlarmList +" <<<<<<<<<<<<");
			...
			rcaResult = ruleResultSetting.setRcaResult(roadmRcaResult);
		} catch (Exception e) {
			LOGGER.error(">>>>>>>[RoadmSingleRule] roadmAlarmRule error : " + ExceptionUtils.getStackTrace(e)+" <<<<<<<<<<<<");
		} finally {
