package com.nia.tsdn.service.result.vo;

import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import org.springframework.context.annotation.Scope;
import org.springframework.stereotype.Component;

import java.io.Serializable;
import java.util.List;

@JsonInclude(JsonInclude.Include.NON_NULL)
@JsonIgnoreProperties(ignoreUnknown = true)
@Component()
@Scope(value = "prototype")
public class PceOutputDataVo implements Serializable {

    @JsonProperty("src-tp-id")
    private String srcTpId;
    @JsonProperty("topology-id")
    private String topologyId;
    @JsonProperty("pce")
    private List<PceListVo> pceListVo;
    @JsonProperty("src-node-id")
    private String srcNodeId;
    @JsonProperty("cir")
    private int cir;
    @JsonProperty("dst-node-id")
    private String dstNodeId;
    @JsonProperty("count")
    private int count;
    @JsonProperty("dst-tp-id")
    private String dstTpId;
    @JsonProperty("pir")
    private int pir;

    @JsonProperty("src-tp-id")
    public String getSrcTpId() {
        return srcTpId;
    }

    @JsonProperty("src-tp-id")
    public void setSrcTpId(String srcTpId) {
        this.srcTpId = srcTpId;
    }

    @JsonProperty("topology-id")
    public String getTopologyId() {
        return topologyId;
    }

    @JsonProperty("topology-id")
    public void setTopologyId(String topologyId) {
        this.topologyId = topologyId;
    }

    @JsonProperty("pce")
    public List<PceListVo> getPceListVo() {
        return pceListVo;
    }

    @JsonProperty("pce")
    public void setPceListVo(List<PceListVo> pceListVo) {
        this.pceListVo = pceListVo;
    }

    @JsonProperty("src-node-id")
    public String getSrcNodeId() {
        return srcNodeId;
    }

    @JsonProperty("src-node-id")
    public void setSrcNodeId(String srcNodeId) {
        this.srcNodeId = srcNodeId;
    }

    @JsonProperty("cir")
    public int getCir() {
        return cir;
    }

    @JsonProperty("cir")
    public void setCir(int cir) {
        this.cir = cir;
    }

    @JsonProperty("dst-node-id")
    public String getDstNodeId() {
        return dstNodeId;
    }

    @JsonProperty("dst-node-id")
    public void setDstNodeId(String dstNodeId) {
        this.dstNodeId = dstNodeId;
    }

    @JsonProperty("count")
    public int getCount() {
        return count;
    }

    @JsonProperty("count")
    public void setCount(int count) {
        this.count = count;
    }

    @JsonProperty("dst-tp-id")
    public String getDstTpId() {
        return dstTpId;
    }

    @JsonProperty("dst-tp-id")
    public void setDstTpId(String dstTpId) {
        this.dstTpId = dstTpId;
    }

    @JsonProperty("pir")
    public int getPir() {
        return pir;
    }

    @JsonProperty("pir")
    public void setPir(int pir) {
        this.pir = pir;
    }
}
