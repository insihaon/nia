...
public class TEA {
	...
	public TEA(byte[] key) {
		if (key == null)
			throw new IllegalArgumentException("Invalid key: Key was null");
		if (key.length < 16)
			throw new IllegalArgumentException("Invalid key: Length was less than 16 bytes");
	...
	public String encrypt(String plaintext) {
		byte[] base64= null;
		try {
			base64 = java.util.Base64.getEncoder().encode(plaintext.getBytes("UTF-8"));
		} catch (UnsupportedEncodingException e) {
			log.error(e.toString());
		}
		byte[] cipher = encrypt(base64);
	...
	public String decrypt(String ciphertext) {
		...
		try {
			plainText = new String(java.util.Base64.getDecoder().decode(plainTextBytes), "UTF-8");
		} catch (Exception e) {
		}
