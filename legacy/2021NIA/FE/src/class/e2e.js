...
import { exportServicelist, getServicelist } from '@/min/route'
import $store from '@/store'
import { apis as niaApis } from '@/api/simulation/nia.js'
...
export class E2e {
  ...
  constructor() {
    ...
    this.self = this
    this.autoExport = (process.env.NODE_ENV !== 'development')
    ...
    return new Promise(resolve => {
      ...
      Promise.allSettled(arr)
        .then(async result => {
          ...
          const failsCount = this.fails.length
          console.log(`E2E TEST - Result(${start}~${end}): success=${end - start - failsCount}, fail=${failsCount}`)
  ...
  async getData(apis = apis_exam, start = 0, end = 1100, buffer = 100) {
    this.fails.splice(0)
    ...
    for (let index = start; index < end; index += buffer) {
      await this.request(apis, index, Math.min(end, index + buffer))
    ...
    console.log('E2E TEST - FINISH')
  }

  async nia(start = 0, end = 1100, buffer = 100) {
    this.getData(niaApis, start, end, buffer)
  }
