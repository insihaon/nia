...
export default {
  ...
  computed: {
    ...
    currentDate() {
      ...
      return today.substring(0, 10) + 'T' + today.substring(11, 13) + ':' + (currentMinute === 0 ? '00' : currentMinute) + ':00'
    },
  },
  created() {
  ...
  methods: {
      ...
      try {
        ...
        for (let i = 0; i < aiChartData.length; i++) {
          ...
          if (aiData.fit_date >= aiData.ds) {
            ...
            if (this.diffDate(aiData.fit_date, aiData.ds.substring(0, 10)) === 29) {
              if (lastIndex === 0) {
                lastIndex = i
    ...
    async getCurrentValue(aiChartData) {
      // const res = await apiSelectCurrentChartData({ IF_ID: aiChartData[0].ipsdn_if_id, FIT_DATE: aiChartData[0].fit_date })
      // const currentList = res.result
    },

    setThresholdDate(aiChartData) {
      if (aiChartData != null && aiChartData.length > 0) {
        if (!aiChartData[0].predictreachdate || aiChartData[0].predictreachdate === '') {
          this.thresholdDate = '없음(30일 이내)'
        } else {
          this.thresholdDate =
            aiChartData[0].predictreachdate.substring(0, 4) +
            '.' +
            aiChartData[0].predictreachdate.substring(5, 7) +
            '.' +
            aiChartData[0].predictreachdate.substring(8, 10) +
            '(' +
            this.diffDate(aiChartData[0].fitdate, aiChartData[0].predictreachdate) +
            '일후)'
        }
      } else {
        this.thresholdDate = '없음(30일 이내)'
