  ...
  >
    ...
    <div class="dragItemHeader" @dblclick="maximizeWindow()">
      <div class="dragItemHeaderTitle" @mousedown="dragItemMouseDownHandler($event, $store.state.showWidgetList)">
        <span v-if="wdata.showTitle" style="pointer-events:none">
          <i class="el-icon-menu" /> {{ wdata.name }}
        </span>
      </div>

      <!-- 도움말 -->
      <!-- <transition name="el-zoom-in-center">
          <el-tooltip class="item" effect="dark" content="도움말" placement="top">
            <i class="el-icon-question questionBtn" @click="showHelpDesk()" />
          </el-tooltip>
        </transition> -->
      ...
      <!-- 창크기 이전상태로 -->
      <!-- <transition name="el-zoom-in-center">
          <el-tooltip class="item" effect="dark" content="창열기" placement="top">
            <i v-if="wdata.windowState=='minimize'"  class="el-icon-copy-document maximizeBtn" @click="restoreWindow()" />
          </el-tooltip>
        </transition> -->
...
/* eslint-disable */
  export default {
    ...
    methods:{
      ...
      resizeItemMouseDownHandler_b: function(event, edit){
        if( !event.target.classList.contains('resizeHandler_b')){ return; }
        if( this.wdata.windowState == 'minimize'){ return; }
      ...
      resizeItemMouseDownHandler_l: function(event, edit){
        if( !event.target.classList.contains('resizeHandler_l')){ return; }
        if( this.wdata.windowState == 'minimize'){ return; }
      ...
      resizeItemMouseDownHandler_b_l: function(event, edit){
        if( !event.target.classList.contains('resizeHandler_b_l')){ return; }
        if( this.wdata.windowState == 'minimize'){ return; }
      ...
      resizeItemMouseDownHandler_r: function(event, edit){
        if( !event.target.classList.contains('resizeHandler_r')){ return; }
        if( this.wdata.windowState == 'minimize'){ return; }
        this.$set(this.wdata, "windowState", "normal");
        this.resizeActiveItem = event.target.parentElement;
        var posx = parseInt( this.resizeActiveItem.style.width);
      ...
      resizeItemMouseDownHandler: function(event, edit){
        if( !event.target.classList.contains('resizeHandler')){ return; }
        if( this.wdata.windowState == 'minimize'){ return; }
        this.$set(this.wdata, "windowState", "normal");
        this.resizeActiveItem = event.target.parentElement;
        var posx = parseInt( this.resizeActiveItem.style.width);
  ...
  div.dragItem i.resizeHandler:hover{
    color:#000000; transition: all 0.5s; border-bottom:5px solid #3d4048; border-right:5px solid #3d4048;
  }
  ...
  div.dragItem i.resizeHandler_b_l:hover{
    color:#000000; transition: all 0.5s; border-bottom:5px solid #3d4048; border-left:5px solid #3d4048;
  }
