...
import com.kt.ipms.legacy.linkmgmt.providermgmt.neoss.service.NeosstxService;
...
//	private final static String IFODRERR_MSG ="선행 오더 미존재.";
		...
		if( linkUtil.getString(stcRequest.getOfficescode()).isEmpty()
			...
			try{								
				...
				if(linkUtil.getString(stcRequest.getIPADDRESS()).isEmpty()){					
					inputkey = "OFFICESCODE : " +  stcRequest.getOfficescode() + " / ASSIGN_TYPE_CD :  " + stcRequest.getAssignTypeCD();
					...
					inputkey = "OFFICESCODE : " +  stcRequest.getOfficescode() + " / ASSIGN_TYPE_CD :  " + stcRequest.getAssignTypeCD() +" / ipAddressSearchKey : "+stcRequest.getIPADDRESS();
				...
				try{
					PrintLogUtil.printLogInfo("stcResponse.getStcIpSuggestList().size() = "+stcResponse.getStcIpSuggestList().size());
				...
				linkUtil.setSystemlog("SDN_IPM_WS_0001",inputkey,  stcResponse.getResultCD(), stcResponse.getResultMSG());			
			}
		}
		return stcResponse;
	}
		...
		// Check inputParam
		 if (linkUtil.getString(stcRequest.getRegYN()).isEmpty()) {
			...
			try {
				...
				// Set InterFace Logging
				linkUtil.setSystemlog("SDN_IPM_WS_0002", inputkey, stcResponse.getResultCD(), stcResponse.getResultMSG()); 				
			}	
		}
	...
	public Sdn0003Response consumeSdn0003Service(Sdn0003Request stcRequest) {
		...
		if (linkUtil.getString(stcRequest.getSaid()).isEmpty()){
			...
			try{
				...
				// Set InterFace Logging			
				linkUtil.setSystemlog("SDN_IPM_WS_0003", inputkey, stcResponse.getResultCD(), stcResponse.getResultMSG());
			}	
		 }
	...
	public Sdn0004Response consumeSdn0004Service(Sdn0004Request stcRequest) {
		...
		if (linkUtil.getString(stcRequest.getIpBlock()).isEmpty()
			...
			try{
				...
				// Set InterFace Logging			
				linkUtil.setSystemlog("SDN_IPM_WS_0004", inputkey, stcResponse.getResultCD(), stcResponse.getResultMSG());
			}	
		 }
	...
	public Sdn0005Response consumeSdn0005Service(Sdn0005Request stcRequest) {
		...
		// Check inputParam
		 if (linkUtil.getString(stcRequest.getRegYN()).isEmpty()
				 || stcRequest.getStcReservedIPList().size() < 1			
				 ) {
			stcResponse.setResultCD(IFINPUTERR_CD);
			stcResponse.setResultMSG(IFINPUTERR_MSG);
		} else {	
			try {
				...
				// Set InterFace Logging
				linkUtil.setSystemlog("SDN_IPM_WS_0005", inputkey, stcResponse.getResultCD(), stcResponse.getResultMSG()); 				
			}	
		}
