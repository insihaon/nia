...
public class AllocMgmtService {
	...
	public IpAllocOperMstListVo selectListAlcIPMstViaInMstSeq(IpAllocOperMstListVo ipAllocOperMstListVo){
		IpAllocOperMstListVo resultListVo = null;
		if (ipAllocOperMstListVo == null || ipAllocOperMstListVo.getIpAllocOperMstVos() == null || ipAllocOperMstListVo.getIpAllocOperMstVos().size() == 0) {
			throw new ServiceException("CMN.HIGH.00001");
		}
		try {
	...
	public void insertListAllocIPMst(IpAllocMstComplexVo ipAllocMstComplexVo){
		try {
			if (ipAllocMstComplexVo == null || ipAllocMstComplexVo.getSrcIpAllocMstVo() == null 
					|| ipAllocMstComplexVo.getDestIpAllocMstVos() == null || ipAllocMstComplexVo.getDestIpAllocMstVos().size() == 0) {
				throw new ServiceException("CMN.HIGH.00001");
			}
			...
			if(srcIpAllocMstVo.getNipAllocMstSeq() != null){
				...
				for (IpAllocOperMstVo ipAllocOperMstVo : destIpAllocMstVos) {
					...
					if (preSrcIpAllocMstVo.getSsvcLineTypeCd().equals("CL0001") || preSrcIpAllocMstVo.getSsvcLineTypeCd().equals("CL0002")) {
						boolean isAvail = false;
						for (IpmsSvcVo ipmsSvcVo : resultListIpmsSvc) {
							if (ipmsSvcVo.getSsvcLineTypeCd().equals("CL0001") || ipmsSvcVo.getSsvcLineTypeCd().equals("CL0002")) {
								isAvail = true;
								break;
							}
						}
...
//						if(insertIpAllocMstVos.get(i).getsGubun() == null || insertIpAllocMstVos.get(i).getsGubun() == "") { //Codeeyes-Urgent-객체 비교시 == , != 사용제한
						if(insertIpAllocMstVos.get(i).getsGubun() == null || insertIpAllocMstVos.get(i).getsGubun().equals("")) {
							PrintLogUtil.printLog("#####ASSIGN MST SEQ: " + insertIpAllocMstVos.get(i).getNipAssignMstSeq());
			...
			for (TbIpAssignMstVo destHistoryTbIpAssignMstVo : destIpAssignMstVos) {
				...
				if (insertIpAllocMstVos != null && insertIpAllocMstVos.size() > 0) {
					...
					for (TbIpAllocMstVo keyIpAllocOperMstVo : insertIpAllocMstVos) {
						...
						if(keyIpAllocOperMstVo.getNipAssignMstSeq().equals(destHistoryTbIpAssignMstVo.getNipAssignMstSeq())){
							
							srchHistoryTbIpAllocMstVo.setNipAssignMstSeq(destHistoryTbIpAssignMstVo.getNipAssignMstSeq());
		...
		try {
			...
			if(srcIpAllocMstVo.getNipAssignMstSeq() != null && srcIpAllocMstVo.getNipAllocMstSeq() != null){
				...
				for (IpAllocOperMstVo ipAllocOperMstVo : destIpAllocMstVos) {
					...
					deleteIpHistMstVo.setSsubsclgipportdescription(preDtlSrcIpAllocMstVo.getSsubsclgipportdescription());
				...
				for (IpHistoryMstVo destTicketTbIpAssignMstVo : deleteIpHistMstVos) {
					...
					ipHistoryMstVo.setSsubsclgipportdescription(destTicketTbIpAssignMstVo.getSsubsclgipportdescription());
...
//							if(deleteIpAllocMstVos.get(i).getsGubun() == null || deleteIpAllocMstVos.get(i).getsGubun() == "") { //Codeeyes-Urgent-객체 비교시 == , != 사용제한
							if(deleteIpAllocMstVos.get(i).getsGubun() == null || deleteIpAllocMstVos.get(i).getsGubun().equals("")) {
								PrintLogUtil.printLog("#####ASSIGN MST SEQ: " + deleteIpAllocMstVos.get(i).getNipAssignMstSeq());
								TbRoutChkMstVo tbRoutChkMstVo = new TbRoutChkMstVo();
